...
class VisualizadorRed:
    ...
    def draw_coverage(self):
        ...
        for nodo in self.nodos:
            ...
            self.canvas.create_oval(cx - r_scaled, cy - r_scaled, cx + r_scaled, cy + r_scaled, outline=color_cobertura, dash=(4, 4))
        ...
        for i in range(len(self.nodos)):
            for j in range(i + 1, len(self.nodos)):
                if self.neighbors(self.nodos[i], self.nodos[j]):
                    ...
                    self.canvas.create_line(x1, y1, x2, y2, fill="#FFFB00", dash=(8, 6))  #amarillo
